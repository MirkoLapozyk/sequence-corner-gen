...
while IFS= read -r line; do
  if (( line_number % 3 == 1 )); then
    ...
    line2=$line
  elif (( line_number % 3 == 0 )); then
    line3=$line
	../generic_float_calculator-main/build/reference_model $op $line1 $line2 $line3 | awk -F'[()]' '{if (NF>1) print $4}' >> flags.txt
#	../generic_float_calculator-main/build/reference_model $op $line3 $line2 >> flags.txt
	../generic_float_calculator-main/build/reference_model $op $line1 $line2 $line3 | grep -oP '0x[0-9a-fA-F]+' >> hex.txt
  fi
  ((line_number++))
done < "data_in.hex"
